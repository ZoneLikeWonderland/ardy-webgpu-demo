...
OUTPUT=${DMD_DIAGNOSTIC_OUTPUT:-distill_runs/dmd_ladd_u05_multideep_gradient_diagnostics_g350_joint22500_20260716}

export CUDA_VISIBLE_DEVICES=1,2,3,4
export PYTHONPATH=.:ardy
export OMP_NUM_THREADS=4

if [[ -f "$OUTPUT/weights/step-0000350/flow_ema.safetensors" ]]; then
  echo "gradient diagnostic control already complete"
  exit 0
fi
...
accelerate launch \
  ...
  --generator-component-gradient-every 10 \
  --encoder-width 512 \
  --encoder-blocks 4 \
  --decoder-width 512 \
  --decoder-blocks 8 \
