...
SOURCE=distill_runs/checkpoint_transforms/shared_g300_to_independent_deep2/state/step-0000300
ROOT=${DMD_INDEPENDENT_HEADS_ROOT:-distill_runs/dmd_ladd_matched_adv03_independent_heads_from_g300_joint22500_20260716}

export CUDA_VISIBLE_DEVICES=${DMD_INDEPENDENT_HEADS_GPUS:-0,1,2,7}
...
exec 9<"$ROOT"
if ! flock -n 9; then
  echo "independent-head DMD/LADD training or evaluation is already running" >&2
  exit 75
fi
...
run_arm() {
  ...
  find "$output/weights" -type f \( \
    -name flow.safetensors -o \
    -name fake_score.safetensors -o \
    -name critic.safetensors \
  \) -delete
}

run_arm independent_d1 1 29801
run_arm independent_d4 4 29802
