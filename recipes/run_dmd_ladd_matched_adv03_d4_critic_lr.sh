...
ROOT=${DMD_MATCHED_D4_LR_ROOT:-distill_runs/dmd_ladd_matched_adv03_d4_critic_lr_from_g300_joint22500_20260716}

export CUDA_VISIBLE_DEVICES=${DMD_MATCHED_D4_LR_GPUS:-1,2,3,4}
...
exec 9<"$ROOT"
if ! flock -n 9; then
  echo "matched-time D4 critic-LR matrix is already running" >&2
  exit 75
fi
...
run_arm() {
  ...
  accelerate launch \
    ...
    --critic-updates-per-generator 4 \
...
verify_control_reproduction() {
  local reference=distill_runs/dmd_ladd_matched_adv03_critic_ratio_from_g300_joint22500_20260716/g1_s1_d4
  local control="$ROOT/clr1e6_control"
  ...
  for step in 400 500; do
    ...
    if [[ "$actual" != "$expected" ]]; then
      echo "D4 critic-LR control reproduction failed at G=$step: expected=$expected actual=$actual" >&2
      return 1
    fi
    echo "D4 critic-LR control reproduction passed at G=$step: $actual"
  done
}

run_arm clr1e6_control 1e-6 29781
verify_control_reproduction
run_arm clr1e8 1e-8 29782
run_arm clr3e8 3e-8 29783
run_arm clr1e7 1e-7 29784
run_arm clr3e7 3e-7 29785
