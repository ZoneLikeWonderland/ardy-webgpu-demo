...
# The control omits the new multi-tap flag and therefore follows the exact
# historical body_final scalar path. Every arm performs the same S/D 700->800
# recovery before generator updates resume at g300.
run_arm single_body_final 29731 \
  --critic-feature-tap body_final \
  --critic-tap-aggregation mean_loss
run_arm multi_deep_mean_loss 29732 \
  --critic-feature-taps body_mid body_final \
  --critic-tap-aggregation mean_loss \
  --log-critic-tap-gradient-rms
run_arm multi_three_mean_loss 29733 \
  --critic-feature-taps trunk_final body_mid body_final \
  --critic-tap-aggregation mean_loss \
  --log-critic-tap-gradient-rms
run_arm multi_three_mean_logit 29734 \
  --critic-feature-taps trunk_final body_mid body_final \
  --critic-tap-aggregation mean_logit \
  --log-critic-tap-gradient-rms
