...
ROOT=${DMD_MATCHED_LR_ROOT:-distill_runs/dmd_ladd_matched_adv03_generator_lr_to700_joint22500_20260716}

# Keep the physical-device/rank mapping identical to the matched-time control run.
export CUDA_VISIBLE_DEVICES=${DMD_MATCHED_LR_GPUS:-1,2,3,4}
...
exec 9<"$ROOT"
if ! flock -n 9; then
  echo "matched-time generator-LR screen is already running" >&2
  exit 75
fi
...
run_arm() {
  ...
  if [[ -f "$output/weights/step-0000700/flow_ema.safetensors" ]]; then
...
verify_control_reproduction() {
  local old_root=distill_runs/dmd_ladd_u05_multideep_high_adv_matched_time_from_g300_joint22500_20260716/adv3e1
  local new_root="$ROOT/lr1e7_control"
  local step expected actual

  for step in 400 500; do
    expected=$(sha256sum "$old_root/weights/step-0000${step}/flow_ema.safetensors" | awk '{print $1}')
    actual=$(sha256sum "$new_root/weights/step-0000${step}/flow_ema.safetensors" | awk '{print $1}')
    if [[ "$actual" != "$expected" ]]; then
      echo "control reproduction failed at G=$step: expected=$expected actual=$actual" >&2
      return 1
    fi
    echo "control reproduction passed at G=$step: $actual"
  done
}

# Run the exact 1e-7 control first. The hash gate prevents comparing a matrix
# whose supposedly fixed stochastic trajectory was not actually reproduced.
run_arm lr1e7_control 1e-7 29761
verify_control_reproduction

run_arm lr1e8 1e-8 29762
run_arm lr3e8 3e-8 29763
run_arm lr3e7 3e-7 29764
run_arm lr1e6 1e-6 29765
